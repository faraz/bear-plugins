...
(function () {
  ...
  function renderField(context, state) {
    ...
    var baseSize = width <= 767 ? 1.35 : 1.65;
    ...
    for (y = offset; y < height; y += step) {
      for (x = offset; x < width; x += step) {
        ...
        var alpha = clamp(0.24 + (lift * 0.42), 0.22, 0.82);
        var size = clamp(baseSize + (lift * 1.95), 1.2, step - 4);
        var roundedSize = Math.max(1, Math.round(size));
        var left = Math.round(x - (roundedSize * 0.5));
        var top = Math.round(y - (roundedSize * 0.5));
        var red = Math.round(102 + (lift * 104) + (warmBias * 28) + (coolBias * 8));
        var green = Math.round(97 + (lift * 92) + (warmBias * 18) + (coolBias * 10));
        var blue = Math.round(92 + (lift * 80) + (warmBias * 4) + (coolBias * 25));
  ...
  function initField() {
    ...
    function resizeCanvas() {
      ...
      state.height = height;
      state.dpr = dpr;
      state.step = width <= 767 ? 15 : width <= 1100 ? 13 : 12;
