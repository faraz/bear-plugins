...
(function () {
  ...
  function renderField(context, state) {
    ...
    var baseSize = width <= 767 ? 1.2 : 1.4;
    ...
    for (y = offset; y < height; y += step) {
      for (x = offset; x < width; x += step) {
        ...
        var alpha = clamp(0.26 + (lift * 0.4), 0.24, 0.84);
        var size = clamp(baseSize + (lift * 1.85), 1.1, step - 4);
        var roundedSize = Math.max(1, Math.round(size));
        var left = Math.round(x - (roundedSize * 0.5));
        var top = Math.round(y - (roundedSize * 0.5));
        var red = Math.round(116 + (lift * 98) + (warmBias * 26) + (coolBias * 8));
        var green = Math.round(111 + (lift * 88) + (warmBias * 16) + (coolBias * 10));
        var blue = Math.round(106 + (lift * 76) + (warmBias * 4) + (coolBias * 24));
  ...
  function initField() {
    ...
    function resizeCanvas() {
      ...
      state.height = height;
      state.dpr = dpr;
      state.step = width <= 767 ? 13 : width <= 1100 ? 11 : 10;
